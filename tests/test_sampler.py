...
def test_sampler_invalid_parameters() -> None:
    ...
    sampler = MultiWalkerMetropolis(wavefunction=wf, num_walkers=10, step_size=0.5)
    with pytest.raises(ValueError):
        sampler.sample(num_steps=0, thermalization=10)
    with pytest.raises(ValueError):
        sampler.sample(num_steps=10, thermalization=-1)

def test_acceptance_rate_range() -> None:
